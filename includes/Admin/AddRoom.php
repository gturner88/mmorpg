...
function move(roomid) {
  ...
  {document.getElementById('Map_' + roomid).innerHTML=xmlhttp.responseText}
 }
 xmlhttp.open("GET",'Admin.php?Type=AddRooms&json=1&section=<?php echo $section; ?>&move=' + roomid,true);
 xmlhttp.send(null);
}
</script>
<script type="text/javascript">
function editit(roomid) 
{
	document.getElementById('MRoomid').value = roomid;
	document.getElementById('thediv').style.display = "";
	document.getElementById('roominfo').innerHTML = "Room ID: " + roomid;
}
</script>
<script type="text/javascript">
function remove() {
 ...
 xmlhttp.open("GET",'Admin.php?Type=AddRooms&json=1&section=<?php echo $section; ?>&remove=' + roomid,true);
 xmlhttp.send(null);
}
</script>
<script type="text/javascript">
function SectionTo() {
	 roomid = document.getElementById('MRoomid').value;
 roomto = document.getElementById('RoomTo').value;
	if (window.XMLHttpRequest)
  {// code for IE7+, Firefox, Chrome, Opera, Safari
   xmlhttp=new XMLHttpRequest();
  }
 ...
 xmlhttp.open("GET",'Admin.php?Type=AddRooms&json=1&section=<?php echo $section; ?>&moveto=' + roomto + '&roomid=' + roomid,true);
 xmlhttp.send(null);
}
</script>
<div id="roominfo"></div>
<div id="thediv" style="display:none;">
<input id="Msection" type=hidden value="<?php echo $section; ?>" ><input id="MRoomid" type=hidden value="" />
To section: <input type=text id=RoomTo><input type=submit onclick="SectionTo()" value=Go><br />
Remove Move property: <input type=submit onclick="remove()" value=Go >
</div>
<?php
if(!isset($section))
{
	echo "<form method=get>Section : <select name=section>
			";
	for($i=1;$i < 100; $i++)
    {
	...
	echo "</option><input type=submit value=Submit!><input type=\"hidden\" name=\"Type\" value=\"AddRooms\"></form>";
	exit();
}
if (!is_file("images/MapSec/Section".$section.".jpg")) {
    $action = $_GET[action];
...
copy ($_FILES['imagefile']['tmp_name'], "images/MapSec/Section".$section.".jpg") or die ("Could not copy");
print "Image Uploaded Successfully<br>$size[0] x $size[1]<br>".$imagesize."Kb(".$k." Bytes)";
$lat = 0;
$long = 0;
$roomname = $_POST[name];
$roomimage = 'images/Map/T_ground/TGdesert.jpg';
$filename = 'images/MapSec/Section'.$section.'.jpg';
list($width, $height) = getimagesize($filename);
if (!is_dir("images/Map/Section".$section)) {
    mkdir("images/Map/Section".$section);
}
$cc = new canvasCrop();
$cc->loadImage($filename);
for($i=0;$i < $height; $i = $i + 25)
{
	$lat = $lat + 1;
	$long = 0;
	for($j=0;$j < $width; $j = $j + 25)
	{
		$long = $long + 1;
		$cc->cropToDimensions($j, $i, $j + 25, $i + 25);
		$cc->saveImage('images/Map/Section'.$section.'/Section'.$section.'-'.$lat.'-'.$long.'.jpg');
		mysql_query("INSERT INTO Rooms (`Lat`,`Long`,`Room_name`,`Room_image`,`Section`)VALUES('".$lat."','".$long."','".$roomname."','".$roomimage."', '".$section."')");
	}
}
}
if (!is_file("images/MapSec/Section".$section.".jpg")) {
...
<form method=post action=?Type=AddRooms&action=upload&section=".$section." enctype=multipart/form-data>
Section Image: <input type=file name=imagefile><br>Room Name: <input type=text name=name>
<br>
<input type=submit name=Submit value=Submit><br></form>
";
}
}    
	$latmax = mysql_num_rows(mysql_query("SELECT * FROM `Rooms` WHERE `Section`='".$section."' AND `Long`='1'"));
	$longmax = mysql_num_rows(mysql_query("SELECT * FROM `Rooms` WHERE `Section`='".$section."' AND `Lat`='1'"));
	echo '<table cellpadding="0" cellspacing="0">
	';
	for($i=1;$i < $latmax + 1; $i++)
    {
		echo "<tr>
			";		
		for($j=1;$j < $longmax; $j++)
		{
			$room = mysql_fetch_array(mysql_query("SELECT * FROM `Rooms` WHERE `Section`='".$section."' AND `Lat`='".$i."' AND `Long`='".$j."'"));
			if($room[moveTo] >= 1) {
		 	$movable = "S";
			echo '<td id="Map_'.$room[id].'" onclick="editit(\''.$room[id].'\',\''.$section.'\')" align=center valign=center style="border:1px solid red;width:25px;height:25px;background-image:URL(\'images/Map/Section'.$section.'/Section'.$section.'-'.$i.'-'.$j.'.jpg\')">'.$movable.'</td>
			';
			$movable = "";
			}	
			elseif($room[movable] == "yes") {
		 	$movable = "M";
			echo '<td id="Map_'.$room[id].'" onclick="editit(\''.$room[id].'\',\''.$section.'\')" align=center valign=center style="border:1px solid red;width:25px;height:25px;background-image:URL(\'images/Map/Section'.$section.'/Section'.$section.'-'.$i.'-'.$j.'.jpg\')">'.$movable.'</td>
			';
			$movable = "";
			...
			{
				$movable = "";
				echo '<td id="Map_'.$room[id].'" onclick="move(\''.$room[id].'\',\''.$section.'\')" align=center valign=center style="border:1px solid red;width:25px;height:25px;background-image:URL(\'images/Map/Section'.$section.'/Section'.$section.'-'.$i.'-'.$j.'.jpg\')">'.$movable.'</td>
				';
				$movable = "";
			}
		}
