...
$onhold = $_GET[onhold];
if ($working) {
  mysql_query("UPDATE DB_Main.ToDoList SET Working='".$stat[name]."' WHERE id='".$working."'");
  echo "You Have set your current project<br>";
}
if ($onhold) {
  mysql_query("UPDATE DB_Main.ToDoList SET Working='On Hold' WHERE id='".$onhold."'");
  echo "You Have set your current project<br>";
}
if ($markdone) {
  mysql_query("UPDATE DB_Main.ToDoList SET Done='yes' WHERE id=$markdone");
...
if ($addit) {
  $Desc = $_POST[Desc];
  $Admin = $_POST[Admin];
  $sql="INSERT INTO DB_Main.ToDoList (`Desc`, `Admin`) VALUES ('$Desc', '$Admin')";
...
if ($cpriority) {
  $prior = $_POST[prior];
  mysql_query("update DB_Main.ToDoList set Admin = '".$prior."' where id='".$cpriority."'");
}
echo "<a href='?Type=todolist&add=yes'>Add New Job</a>";

echo "<table bgcolor=lightgreen border=1>";
$result = mysql_query("Select * from DB_Main.ToDoList order by Done, Admin, id Asc");
